...
export const getConversations = async (filters = {}) => {
  const response = await api.get('/api/conversations', { params: { limit: 100, ...filters } });
...
export const submitFeedback = async (conversationId, feedback) => {
  const response = await api.post(`/api/conversations/${conversationId}/feedback`, feedback);
  return response.data;
};

// Analytics
export const getAnalyticsOverview = async (timeRange = 'week') => {
  const response = await api.get('/api/analytics/overview', { params: { timeRange } });
  return response.data;
};
...
export const updateTemplate = async (id, updates) => {
  const response = await api.patch(`/api/templates/${id}`, updates);
  return response.data;
};
